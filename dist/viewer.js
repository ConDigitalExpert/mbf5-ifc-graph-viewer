...
function saveCurrentView() { if (!state.viewer) return; const name = window.prompt("Name this camera view", `View ${state.savedViews.length + 1}`); if (!name?.trim()) return; state.savedViews.push({ name: name.trim(), ...captureView() }); localStorage.setItem(SAVED_VIEWS_KEY, JSON.stringify(state.savedViews)); renderSavedViews(); els.savedViewSelect.value = String(state.savedViews.length - 1); showToast(`Saved view · ${name.trim()}`); }
